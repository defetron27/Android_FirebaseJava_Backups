            ...
            if (requestCode == 1)
            {
                ...
                if (uri != null)
                {
                    ...
                    if (cursor != null)
                    {
                        ...
                        // change face detector bar code detector only
                        ...
                        detector.detectInImage(image).addOnSuccessListener(new OnSuccessListener<List<FirebaseVisionBarcode>>()
                        {
                            ...
                            {
                                stringBuilder.append("Barcode details : ");

                                for(FirebaseVisionBarcode barcode : firebaseVisionBarcodes)
                                {
                                    stringBuilder.append("Raw value : ").append(barcode.getRawValue()).append(",\n");

                                    switch(barcode.getValueType())
                                    {
                                        case FirebaseVisionBarcode.TYPE_WIFI:
                                            stringBuilder.append("SSID : ").append(barcode.getWifi().getSsid()).append("Password : ")
                                                    .append(barcode.getWifi().getPassword()).append("Encryption Type : ").append(barcode.getWifi().getEncryptionType()).append("\n");
                                            break;
                                        case FirebaseVisionBarcode.TYPE_URL:
                                            stringBuilder.append("Title : ").append(barcode.getUrl().getTitle())
                                                    .append("URL : ")
                                                    .append(barcode.getUrl().getUrl()).append("\n");
                                            break;
                                    }
                                }
